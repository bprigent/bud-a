...
A local-first household budgeting app powered by Claude Code. Combines a web UI, a Python API, and AI agents to track spending, parse receipts, and generate financial insights — all without cloud storage or a database.

**Who it is for:** anyone who wants a simple, transparent budgeting stack they control — clone the repo, add your own `data/` CSVs, and run the stack locally.

## Web App

### Dashboard
Monthly overview showing total income, expenses, net balance, account balances, spending by category, and budget progress bars with alerts when over budget. Switch months with the date picker.
...
### Accounts
Manage bank accounts — checking, savings, credit, and investment. Upload bank logos and track opening and statement balances.

### Categories
Manage spending and income categories with emoji icons and descriptions used for AI auto-categorization.

### Settings
Central settings page with links to Members, Accounts, and Categories management.

### Privacy Mode
Toggle privacy mode to redact all amounts in the UI — useful when sharing your screen.

### Real-Time Sync
The frontend syncs instantly with the backend via WebSocket. Changes made through the API, CLI, or Telegram appear in the UI without refreshing.
...
./start.sh
```
Installs dependencies when needed and starts both servers. On first run, it prompts for your default currency (e.g. EUR, USD, GBP) and creates the `.env` files automatically. The UI is at **http://localhost:5173**.
